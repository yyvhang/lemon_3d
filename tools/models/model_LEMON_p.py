...
from tools.models.PointNet import Pts_Encoder_curvature, Pts_Upsample
...
class Decoder(nn.Module):
    def __init__(self, feat_dim, mid_dim=768):
        super().__init__()
        class SwapAxes(nn.Module):
            def __init__(self):
                super().__init__()
        ...
        self.obj_up = Pts_Upsample()
        self.hm_up = Pts_Upsample()
        ...
        self.spatial_head = nn.Sequential(
            nn.Linear(mid_dim, mid_dim//6),
            SwapAxes(),
            nn.BatchNorm1d(mid_dim//6),
            SwapAxes(),
            nn.ReLU(),
            nn.Linear(mid_dim//6, 1)
        )
        ...
        self.content = nn.Sequential(
            ...
            nn.BatchNorm1d(mid_dim//6),
            nn.ReLU(),
            nn.Linear(mid_dim//6, 17)           
        )

        self.contact_up_fine = nn.Linear(1723, 6890)
        self.sigmoid = nn.Sigmoid()

    def forward(self, phi_a, phi_c, phi_p, obj_pts, obj_feats, hm_pts, hm_feats, semantic_feats):

        obj_feats[-1] = phi_a.mT
        obj = self.obj_up(obj_pts, obj_feats)                      
        hm_feats[-1] = phi_c.mT
        hm = self.hm_up(hm_pts, hm_feats)                            

        affordance = self.aff_head(obj.mT)                         
        affordance = self.sigmoid(affordance)

        contact_coarse = self.contact_head(hm.mT)
...
class LEMON(nn.Module):
    def __init__(self, feat_dim, run_type, device):
        ...
        self.vertex_sampler = get_sample(device = device)
        self.img_encoder = Enc_I(run_type, feat_dim)
        self.obj_encoder = Pts_Encoder_curvature()
        self.hm_encoder = Pts_Encoder_curvature()
    ...
    def forward(self, I, O, H, C_h, C_o, meta_masks=None):
        ...
        obj_pts, F_o, C_o = self.obj_encoder(O, C_o)                                    
        hm_pts, F_h, C_h = self.hm_encoder(H.mT, C_h) 

        T_o_, T_h_, F_o_, F_h_, varphi = self.Intention(F_i, F_o[-1].mT, F_h[-1].mT)

        F_co, F_ch, phi_a, phi_c = self.Geometry_Correlation(C_h, C_o, F_o_, F_h_, T_o_, T_h_)
        phi_p = self.Spatial(F_co, F_ch, phi_c, T_o_, T_h_)                  

        contact, affordance, spatial, semantic = self.decoder(phi_a, phi_c, phi_p, obj_pts, F_o, hm_pts, F_h, semantic_feats)
